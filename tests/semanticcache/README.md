...
- **Providers configured with API keys**:
  - **OpenAI** — required. Must have a chat model (default `openai/gpt-4o-mini`) and the embedding model `text-embedding-3-small` (used in Phase 2).
  - **Gemini** — required for cross-provider cases. Chat model: default `gemini/gemini-2.5-flash`.
  - **Anthropic** — required for cross-provider cases. Chat model: default `anthropic/claude-haiku-4-5`.
