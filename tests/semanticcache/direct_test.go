...
func TestDirect(t *testing.T) {
	...
	// suppressed by the header. Expected: zero caching activity, no cache_debug.
	// If the plugin stamps cache_debug or writes an orphan entry, the test fails
	// (which would surface the bug — current behavior is to write orphans).
	...
	t.Run("1.44_plugin_get_status", func(t *testing.T) {
		...
		if got := p.Status.Status; got != "active" {
			t.Fatalf("expected plugin status=active, got %q", got)
		}
